...
class projectModel extends baseModel {
  ...
  getBaseInfo(id, select) {
    select
      = select
      || '_id uid name basepath switch_notice desc group_id project_type env icon color logo add_time up_time pre_script after_script project_mock_script is_mock_open strice is_json5 tag'
    return this.model
      .findOne({
        _id: id,
      })
  ...
  list(group_id) {
    ...
    return this.model
      .find(params)
      .select(
        '_id uid name basepath switch_notice desc group_id project_type color icon logo env add_time up_time',
      )
      .sort({_id: -1})
      .exec()
  }
  ...
  countWithPublic(group_id) {
    const params = {group_id: group_id, project_type: 'public'}
