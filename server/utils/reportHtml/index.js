...
function createHtml5(left, tp, msg, runTime) {
  ...
  if (msg.failedNum === 0) {
    message += `<div>一共 <span class="success">${
      msg.successNum
    }</span> 測試用例， 全部驗證通過(${runTime})</div>`;
  } else {
    message += `<div>一共 ${msg.len} 測試用例，<span class="success"> ${
      msg.successNum
    }</span> 個驗證通過， ${msg.failedNum} 個未通過(${runTime})</div>`;
  }
...
function validHtml(validRes) {
  ...
  <div>
    <div class="row case-report">
     <div class="col-3 case-report-title">驗證結果</div>
     <div class="col-21">
      ${validRes}
     </div>
    </div>
...
function codeHtml(code) {
  ...
  switch (code) {
    case 0:
      codeHtml += `<div title="驗證通過" class="status status-ok"><i class="icon icon-check-circle"></i></div>`;
      break;
    case 400:
      codeHtml += `<div title="請求異常" class="status status-ko"><i class="icon icon-close-circle"></i></div>`;
      break;
    case 1:
      codeHtml += `<div title="驗證失敗" class="status status-warning"><i class="icon icon-warning-circle"></i></div>`;
      break;
    default:
      codeHtml += `<div title="驗證通過" class="status status-warning"><i class="icon icon-warning-circle"></i></div>`;
      break;
  }
  return codeHtml;
}
